    ...
    <div v-if="showHelp" class="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
      ...
      <div class="space-y-4 text-sm text-blue-800">
        <div>
          ...
          <p>
            This dashboard tracks and classifies all <strong>unresolved bugs</strong> in the
            RHOAIENG Jira project. It helps engineering leaders spot patterns — are we accumulating
            regressions? Are usability issues piling up? — without manually triaging hundreds of
            bugs.
          </p>
        </div>
        <div>
          ...
          <p>
            All bugs in the <strong>RHOAIENG</strong> project with
            <code class="bg-blue-100 px-1 rounded">resolution = Unresolved</code>. Resolved, closed,
            and done bugs are excluded. Data is refreshed on demand via the Refresh button.
          </p>
        </div>
        <div>
          ...
          <ol class="list-decimal ml-5 mt-1 space-y-1">
            <li>
              <strong>Rule-based (instant, free)</strong> — Bugs with "regression" labels or
              keywords are classified as <em>Regression</em>. Bugs with UX/accessibility labels or
              UXD components are classified as <em>Usability</em>.
            </li>
            <li>
              <strong>LLM-assisted (Claude Haiku on Vertex AI)</strong> — Remaining bugs are
              analyzed by an LLM that reads the summary and description, then classifies them as
              <em>General Engineering</em> (logic errors, crashes, performance) or
              <em>Uncategorized</em> (insufficient information).
            </li>
          </ol>
        </div>
        <div>
          ...
          <p>
            <strong>Refresh</strong> fetches fresh data from Jira and only reclassifies bugs that
            have been updated since last classification. <strong>Hard Refresh</strong> reclassifies
            every bug from scratch — useful after rule changes or to reset LLM classifications.
          </p>
        </div>
      </div>
    </div>
          ...
          >
            <div class="flex items-center gap-1.5">
              ...
              <div class="relative mb-1">
                <svg
                  class="h-3.5 w-3.5 text-gray-400 cursor-help"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                ...
                <div
                  class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 bg-gray-900 text-white text-xs rounded-lg p-2.5 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10 pointer-events-none"
                >
...
export default {
  ...
  computed: {
    sortedVersions() {
      ...
      return Object.entries(this.summary.byVersion).sort((a, b) => {
        // "Unset" always last
  ...
  methods: {
    ...
    categoryDescription(category) {
      const descriptions = {
        regression:
          'Bugs that broke previously working functionality. Detected via "regression" labels or keywords.',
        usability:
          'UI/UX issues, confusing workflows, and accessibility problems. Detected via UX labels or components.',
        'general-engineering':
          'Logic errors, crashes, performance issues, and missing validation. Classified by LLM analysis.',
